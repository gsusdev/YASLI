...
boolean FtpServer::processCommand()
{
  ...
  else if( ! strcmp( command, "DELE" ))
  {
    ...
    {
	    if (!_fileSystem_p->exists(path))
        client.println( "550 File " + String(parameters) + " not found");
      else
      {
	      if (_fileSystem_p->remove(path))
  ...
  else if( ! strcmp( command, "LIST" ))
  {
    ...
    else
    {
      ...
      uint16_t nm = 0;
#ifdef ESP8266
	    Dir dir = _fileSystem_p->openDir(cwdName);
	    // if( !_fileSystem_p->exists(cwdName))
...
#elif defined ESP32
	    File root = _fileSystem_p->open(cwdName);
			if(!root){
					client.println( "550 Can't open directory " + String(cwdName) );
					// return;
			} else {
  ...
  else if( ! strcmp( command, "RETR" ))
  {
	...
	{
		file = _fileSystem_p->open(path, "r");
      if( !file)
        client.println( "550 File " +String(parameters)+ " not found");
      else if( !file )
  ...
  else if( ! strcmp( command, "STOR" ))
  {
    ...
    {
	    file = _fileSystem_p->open(path, "w");
      if( !file)
    ...
    else if( makePath( path ))
    {
      ...
      {          
		  ...
		  Serial.println("Renaming " + String(buf) + " to " + String(path));
            #endif
	      if (_fileSystem_p->rename(buf, path))
